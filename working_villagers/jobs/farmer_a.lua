...
local farmera_new_start = true
local farmera_last_animation = "STAND"
...
local farmera_marker = nil



local farmera_path_data = nil	-- the builders private path_data


local farmera_searching_range = {x = 20, y = 5, z = 20}
local farmera_searching_distance = 500
local farmera_is_building = false



-- TODO MESSAGING MESSAGES TO BE UPDATED

local farmera_callingmessage = 	{ 
			msg = "Calling Builder A",
			}
local farmera_callingreply = 	{ 
			msg = "Builder A OK",
			}
local farmera_jobreply = 	{ 
			msg = "Builder Finished",
			}
local farmera_comemessage = 	{ 
			msg = "Come Here",
			pos = "",
			}
local farmera_buildmessage = 	{ 
			msg = "Build This",
			pos = "",
			}
local farmera_moveinmessage = 	{ 
			msg = "Movein Here",
			}
...
--	print("MYPOS=", mypos)
--	print("MYDEST=", mydest)
	local farmera_myd = mydest
	farmera_path_data = pathfinder.plot_movement_to_pos(mypos, farmera_myd, false)
	if farmera_path_data == nil then
		-- TODO should indicate who cannot find a path
		print(" No Path Found to ", farmera_myd)
		self.job_data["pathdata"] = {}
		self.job_data["pathstep"] = -1
		return false
	elseif farmera_path_data == false then
...
--		print("          PATHDATA: ",  farmera_path_data)
		self.job_data["pathdata"] = farmera_path_data
		self.job_data["pathstep"] = 1
		return true
...
local function farmera_plantonextto(mypos,mydest)
	local farmera_myd = func.get_closest_clear_spot(mypos,mydest)	
	if farmera_myd == false or farmera_myd == nil then
		farmera_myd = mydest
	end
	farmera_path_data = pathfinder.plot_movement_to_pos(mypos, farmera_myd, false)
	if farmera_path_data == nil then
...
local function go_to_here(spos,epos,self)
	...
	while ma_tries > 0 do
		...
		if thedist > 2 then
			farmera_plantonextto(curr_loc,epos)
			self:go_to_the(farmera_path_data)
		else
			return true
		end
		ma_tries = ma_tries - 1
...
local function check_farmera_message(self)

	--print("builder checking messages")
	local themessage = nil
	if func.get_farmer_a_message() ~= nil then 
		-- there is a message
		themessage = func.get_farmer_a_message()

		if themessage.msg == farmera_callingmessage.msg then
			-- I must answer the phone
			print("farmera IS ANSWERING THE PHONE")
			func.set_farmer_a_message(farmera_callingreply)
		end



		if themessage.msg == farmera_comemessage.msg then
			-- I must answer the phone
			print("farmera IS BEING CALLED OVER")
			farmera_commanddestination = themessage.pos	
			func.set_farmer_a_message(farmera_callingreply)
		local goto_job = {
			["name"] = "gotohere",
			["dest"] = themessage.pos,
		...
		if themessage.msg == farmera_buildmessage.msg then
			print("farmera IS BEING TOLD TO FARM")
...
--		end
		...
		if themessage.msg == farmera_moveinmessage.msg then
			print("farmera: IS BEING TOLD TO MOVEIN")
			...
			print("Farmer: Chest POS = ", self.pos_data.chest_pos)
			print("Farmer: Bed POS = ", self.pos_data.bed_pos)
			print("Farmer: Home POS = ", self.pos_data.home_pos)
			--print("farmera:DUMP POSDATA", dump(self.pos_data))
...
local function move_in_1(self)
	...
	farmera_moveinmessage.bed_pos = bposb
	farmera_moveinmessage.food_pos = cposb
	farmera_moveinmessage.storage_pos = cposb
	farmera_moveinmessage.job_pos = fpos
	farmera_moveinmessage.tools_pos = cposb
	farmera_moveinmessage.chest_pos = cposb
	farmera_moveinmessage.home_pos = hposb
	--print("MAYORA: DUMP MESSAGE = ", dump(farmera_moveinmessage))
	func.set_builder_a_message(farmera_moveinmessage)
...
local function check_build_item(self,initem)
		...
		--			print("I Need a Bed Top")
					local farmera_inv = self:get_inventory()
					if farmera_inv:room_for_item("main", ItemStack(initem)) then
		--				print("adding a bed top")
						farmera_inv:add_item("main", ItemStack(initem))
					else
						local msg = "farmera at " .. minetest.pos_to_string(self.object:get_pos()) ..
...
--					if farmera_inv:room_for_item("main", ItemStack(initem)) then
--						farmera_inv:add_item("main", ItemStack(initem))
--						--farmera_inv:add_item("main", ItemStack(initem))"beds:bed_top"
--					else
--						local msg = "farmera at " .. minetest.pos_to_string(self.object:get_pos()) ..
--							" doesn't have enough inventory space"
--						if self.owner_name then
...
--		print("FARMER: Start Farm This")
	...
	elseif current_job.status == 1 then
--		print("FARMER: Check Next position is within field")
...
--		print("FARMER: POSITION check it for plant")
--		print("DUMP CJOB:" , dump(current_job))
...
--		print("FARMER: PLACEMENTLOC :",locx,locy,locz)
	...
	elseif current_job.status == 3 then
		-- goto plant
--		print("FARMER: MOVING TO POSITION")
--		print("DUMP CJOB:" , dump(current_job))
...
--		print("DUMP CJOB:" , dump(current_job))
		...
		if plant_data and plant_data.replant then
			...
			for index, value in ipairs(plant_data.replant) do
				self:place(value, vector.add(locp, vector.new(0,index-1,0)))
			end
		end
...
local function check_inventory_for_space(self)
					local farmera_inv = self:get_inventory()
					--print("DUMP BUILDER INVENTORY: ", dump(farmera_inv))
					if farmera_inv:room_for_item("main", "dummy:unknown") then
						return true
					else
						return false
					end
...
--[[			local mybl = vector.add(get_job_position(self),farmera_town_locations[1])
			local myml = vector.add(get_job_position(self),farmera_town_markers[1])
			local farmera_meta = minetest.get_meta(myml)

			local special_job = {
				["name"] = "movein1",
				["status"] = 0,
				["buildpos"] = mybl,
				["x"] = farmera_meta:get_int("maxx"),
				["y"] = farmera_meta:get_int("maxy"),
				["z"] = farmera_meta:get_int("maxz"),
			}
...
-- ONLY ON GAME LOAD AND CONTINUE
		if farmera_new_start == true then
...
-- print("JOBLIST=", dump(farmera_joblist))
...
--print("farmera JOBS = ", get_size_joblist(self))
