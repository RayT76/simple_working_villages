...
local vet_new_start = true
local vet_last_animation = "STAND"
...
local vet_marker = nil



local vet_path_data = nil	-- the builders private path_data


local vet_searching_range = {x = 20, y = 5, z = 20}
local vet_searching_distance = 500
local vet_is_building = false



-- TODO MESSAGING MESSAGES TO BE UPDATED

local vet_callingmessage = 	{ 
			msg = "Calling Builder A",
			}
local vet_callingreply = 	{ 
			msg = "Builder A OK",
			}
local vet_jobreply = 	{ 
			msg = "Builder Finished",
			}
local vet_comemessage = 	{ 
			msg = "Come Here",
			pos = "",
			}
local vet_buildmessage = 	{ 
			msg = "Build This",
			pos = "",
			}
...
--	print("MYPOS=", mypos)
--	print("MYDEST=", mydest)
	local vet_myd = mydest
	vet_path_data = pathfinder.plot_movement_to_pos(mypos, vet_myd, false)
	if vet_path_data == nil then
		-- TODO should indicate who cannot find a path
		print(" No Path Found to ", vet_myd)
		self.job_data["pathdata"] = {}
		self.job_data["pathstep"] = -1
		return false
	elseif vet_path_data == false then
...
--		print("          PATHDATA: ",  vet_path_data)
		self.job_data["pathdata"] = vet_path_data
		self.job_data["pathstep"] = 1
		return true
...
local function vet_plantonextto(mypos,mydest)
	local vet_myd = func.get_closest_clear_spot(mypos,mydest)	
	if vet_myd == false or vet_myd == nil then
		vet_myd = mydest
	end
	vet_path_data = pathfinder.plot_movement_to_pos(mypos, vet_myd, false)
	if vet_path_data == nil then
...
local function go_to_here(spos,epos,self)
	...
	while ma_tries > 0 do
		...
		if thedist > 2 then
			vet_plantonextto(curr_loc,epos)
			self:go_to_the(vet_path_data)
		else
			return true
		end
		ma_tries = ma_tries - 1
...
local function check_vet_message(self)

	--print("builder checking messages")
	local themessage = nil
	if func.get_vet_a_message() ~= nil then 
		-- there is a message
		themessage = func.get_vet_a_message()

		if themessage.msg == vet_callingmessage.msg then
			-- I must answer the phone
			print("vet IS ANSWERING THE PHONE")
			func.set_vet_a_message(vet_callingreply)
		end



		if themessage.msg == vet_comemessage.msg then
			-- I must answer the phone
			print("vet IS BEING CALLED OVER")
			vet_commanddestination = themessage.pos	
			func.st_vet_a_message(vet_callingreply)
		local goto_job = {
			["name"] = "gotohere",
			["dest"] = themessage.pos,
			["status"] = 0
		...
		if themessage.msg == vet_buildmessage.msg then
			print("vet IS BEING TOLD TO FARM")
				local plantorchard_job = themessage
				plantorchard_job["name"] = "buildthis"
				add_to_joblist(self,plantorchard_job)
			func.get_vet_a_message(vet_callingreply)
		end


			vet_marker = themessage.pos	
...
--		end
		...
		if themessage.msg == vet_moveinmessage.msg then
			print("vet: IS BEING TOLD TO MOVEIN")
			...
			print("vet: Chest POS = ", self.pos_data.chest_pos)
			print("vet: Bed POS = ", self.pos_data.bed_pos)
			print("vet: Home POS = ", self.pos_data.home_pos)
			--print("vet:DUMP POSDATA", dump(self.pos_data))
...
local function move_in_1(self)
	...
	vet_moveinmessage.food_pos = cposb
	vet_moveinmessage.storage_pos = cposb
	vet_moveinmessage.job_pos = fpos
	vet_moveinmessage.tools_pos = cposb
	vet_moveinmessage.chest_pos = cposb
	vet_moveinmessage.home_pos = hposb
	--print("MAYORA: DUMP MESSAGE = ", dump(vet_moveinmessage))
	func.set_builder_a_message(vet_moveinmessage)
...
local function find_building(p)
	...
	local vet_meta = minetest.get_meta(p)
	if vet_meta:get_string("state") ~= "begun" then
		return false
	end
	local vet_build_pos = simple_working_villages.buildings.get_build_pos(vet_meta)
	if vet_build_pos == nil then
		return false
	end
	if simple_working_villages.buildings.get(vet_build_pos)==nil then
		return false
	end
	return true
end
...
local function check_build_item(self,initem)
		...
		--			print("I Need a Bed Top")
					local vet_inv = self:get_inventory()
					if vet_inv:room_for_item("main", ItemStack(initem)) then
		--				print("adding a bed top")
						vet_inv:add_item("main", ItemStack(initem))
					else
						local msg = "vet at " .. minetest.pos_to_string(self.object:get_pos()) ..
...
--					if vet_inv:room_for_item("main", ItemStack(initem)) then
--						vet_inv:add_item("main", ItemStack(initem))
--						--vet_inv:add_item("main", ItemStack(initem))"beds:bed_top"
--					else
--						local msg = "vet at " .. minetest.pos_to_string(self.object:get_pos()) ..
--							" doesn't have enough inventory space"
--						if self.owner_name then
...
-- TODO CHANGE THIS TO CUT GRASS ---
	...
	if current_job.status == 0 then
			...
			if dobj ~= nil then
				...
				print("vet:Found a wounded Animal at ", found_npc_target)
        		current_job.status = 1
	        	rem_from_joblist(self)
		        add_to_joblist(self,current_job)
        else
...
local function check_inventory_for_space(self)
					local vet_inv = self:get_inventory()
					--print("DUMP BUILDER INVENTORY: ", dump(vet_inv))
					if vet_inv:room_for_item("main", "dummy:unknown") then
						return true
					else
						return false
					end
...
--[[			local mybl = vector.add(get_job_position(self),vet_town_locations[1])
			local myml = vector.add(get_job_position(self),vet_town_markers[1])
			local vet_meta = minetest.get_meta(myml)

			local special_job = {
				["name"] = "movein1",
				["status"] = 0,
				["buildpos"] = mybl,
				["x"] = vet_meta:get_int("maxx"),
				["y"] = vet_meta:get_int("maxy"),
				["z"] = vet_meta:get_int("maxz"),
			}
...
-- print("JOBLIST=", dump(vet_joblist))
...
--print("vet JOBS = ", get_size_joblist(self))
if get_size_joblist(self) == 0 then 
            print("VET JOB ADDING NOTHING")
			local njob = {
				name = "nothing",
