...
local buildera_new_start = true
local buildera_last_animation = "STAND"
...
local buildera_marker = nil



local buildera_path_data = nil	-- the builders private path_data


local buildera_searching_range = {x = 20, y = 5, z = 20}
local buildera_searching_distance = 500
local buildera_is_building = false



-- TODO MESSAGING MESSAGES TO BE UPDATED

local buildera_callingmessage = 	{ 
			msg = "Calling Builder A",
			}
local buildera_callingreply = 	{ 
			msg = "Builder A OK",
			}
local buildera_jobreply = 	{ 
			msg = "Builder Finished",
			}
local buildera_comemessage = 	{ 
			msg = "Come Here",
			pos = "",
			}
local buildera_buildmessage = 	{ 
			msg = "Build This",
			pos = "",
			}
local buildera_moveinmessage = 	{ 
			msg = "Movein Here",
			}
...
--	print("MYDEST=", mydest)
	...
	buildera_path_data = pathfinder.plot_movement_to_pos(mypos, buildera_myd, false)
	if buildera_path_data == nil then
		...
		return false
	elseif buildera_path_data == false then
...
--		print("          PATHDATA: ",  buildera_path_data)
		self.job_data["pathdata"] = buildera_path_data
		self.job_data["pathstep"] = 1
		return true
...
local function buildera_plantonextto(mypos,mydest)
...
local function go_to_here(spos,epos,self)
	...
	while ma_tries > 0 do
		...
		if thedist > 2 then
			buildera_plantonextto(curr_loc,epos)
			self:go_to_the(buildera_path_data)
		else
			return true
		end
		ma_tries = ma_tries - 1
...
local function check_buildera_message(self)
	...
	if func.get_builder_a_message() ~= nil then 
		...
		if themessage.msg == buildera_callingmessage.msg then
			-- I must answer the phone
			print("BUILDERA IS ANSWERING THE PHONE")
			func.set_builder_a_message(buildera_callingreply)
		end



		if themessage.msg == buildera_comemessage.msg then
			-- I must answer the phone
			print("BUILDERA IS BEING CALLED OVER")
			buildera_commanddestination = themessage.pos	
			func.set_builder_a_message(buildera_callingreply)
		local goto_job = {
			["name"] = "gotohere",
			["dest"] = themessage.pos,
		...
		if themessage.msg == buildera_buildmessage.msg then
			print("BUILDERA IS BEING TOLD TO BUILD")
			buildera_marker = themessage.pos	
			func.set_builder_a_message(buildera_callingreply)
		...
		if themessage.msg == buildera_moveinmessage.msg then
			print("BUILDERA: IS BEING TOLD TO MOVEIN")
			...
			--print("BUILDERA:DUMP POSDATA", dump(self.pos_data))
...
local function move_in_1(self)
	...
	buildera_moveinmessage.bed_pos = bposb
	buildera_moveinmessage.food_pos = cposb
	buildera_moveinmessage.storage_pos = cposb
	buildera_moveinmessage.job_pos = fpos
	buildera_moveinmessage.tools_pos = cposb
	buildera_moveinmessage.chest_pos = cposb
	buildera_moveinmessage.home_pos = hposb
	--print("MAYORA: DUMP MESSAGE = ", dump(buildera_moveinmessage))
	func.set_builder_a_message(buildera_moveinmessage)
...
local function check_build_item(self,initem)
		...
		--				print("adding a bed top")
				...
				-- TODO FIXME needs to be updated to check for bed correctly
...
local function build_this(self)
	local current_job = get_from_joblist(self,1)
...
--	print("BUILDERA: BUILD THIS JOB ", dump(current_job))
	...
	-- I have a building site to build
		local percentcomp = buildera_meta:get_int("index") / #buildera_building_on_pos.nodedata * 100
		local remaining = #buildera_building_on_pos.nodedata - buildera_meta:get_int("index")
		local message = "BUILDER:BUILD " .. percentcomp .. "% Completed ( " .. remaining .. " nodes remaining)"
		print(message)



		--if buildera_building_on_pos.nodedata and (buildera_meta:get_int("index") > #buildera_building_on_pos.nodedata) then
		if (buildera_meta:get_int("index") > #buildera_building_on_pos.nodedata) then
			self:set_state_info("I am currently marking a building as finished.")
			-- TODO use my better function... maybe this works

--			print("BUILDER:STAGE0: I HAVE FINISHED THE BUILD")
			local buildera_destination = func.get_closest_clear_spot(self.object:get_pos(),buildera_marker)	--func.find_adjacent_clear(buildera_marker)
			--buildera_destination = func.find_ground_below(buildera_destination)
			current_job.status = 1
	rem_from_joblist(self)
			add_to_joblist(self,current_job)
		...
		add_to_joblist(self,waitfor_job)
			...
			local goto_joba = {
				["name"] = "gotohere",
				["dest"] = buildera_destination,
				["status"] = 0
			}
			add_to_joblist(self,goto_joba)
		else
--			print("I NEED TO CONTINUE THE BUILD")
...
--			buildera_destination = func.find_ground_below(buildera_destination)
--			if buildera_destination==false then
--				print("failure: no adjacent walkable found")
--				buildera_destination = marker
--			end

--			buildera_path_data = pathfinder.plot_movement_to_pos(self.object:get_pos(), buildera_destination, false)
--			if buildera_path_data == nil then
--				-- TODO should indicate who cannot find a path
--				print(self.object:get_luaentity().nametag, " No Path Found to ", buildera_destination)
--				--return nil
--			elseif buildera_path_data == false then
...
--			end	
	...
	elseif current_job.status == 1 then		
			-- finish build and mark as so
			buildera_marker = nil
--			print("BUILDER:STAGE1: SENDING JOB DONE MESSAGE AND SETTING THE BUILD TO FINISHED")
			func.set_builder_a_message(buildera_jobreply)
			buildera_meta:set_string("state","finished")
			buildera_is_building = false
...
--				print("BUILDER:STAGE3: GOING TO POSITION")--, dump(build_node)) 
				current_job.status = 4
	rem_from_joblist(self)
				add_to_joblist(self,current_job)
				local buildera_npos = build_node.pos

				local gotopos = func.get_closest_clear_spot(self.object:get_pos(),buildera_npos)	

				if gotopos == nil then
--					print("GetClostestClearSpot == nil trying adjacent pos")
					gotopos = func.find_adjacent_clear(buildera_npos)
					print("gotopos = ", gotopos)
					if gotopos == nil then
--						print("GetClostestAdjacentSpot == nil trying node.pos")
						gotopos = buildera_npos
					end
				end
	...
	elseif current_job.status == 4 then
		-- check for placing air ? not too sure its needed
--				print("BUILDER:STAGE4: ")--DUMP BUILD NODE STAGE4a ", dump(build_node)) 
				local buildera_npos = build_node.pos
...
--					print("BUILDER:STAGE5: CHECKING INVENTARY FOR SWAPPABLE ITEMS")
...
--				print("BUILDER:STAGE6: PLACING NODE", buildera_nname) 
				...
				if is_material(wield_stack:get_name()) or self:has_item_in_main(is_material) then
					...
					local buildera_mydestination = func.get_closest_clear_spot(self.object:get_pos(),buildera_npos)	
					if buildera_mydestination == false or buildera_mydestination == nil then
						print("Builder: No clostest clear spot found")
						buildera_mydestination = buildera_npos
					end
					self:set_state_info("I am building.")

					local wnode_name = minetest.get_node(buildera_npos).name				

					if wnode_name ~= build_nodenode.name then

						while minetest.get_node(buildera_npos).name ~= "air" do
							self:dig(buildera_npos,false)
							coroutine.yield()
						end

						if build_nodenode.name ~= "bones:bones" then


							if self:place(build_nodenode,buildera_npos) then
		--						print("Builder: placed ok")
								...
								-- TODO should do some checks to see if the block was actually placed
								-- TODO just ignore for now and continue
								
								print("Builder: Could not place at ",buildera_mydestination, " ???")
								buildera_meta:set_int("index",buildera_meta:get_int("index")+1)
								current_job.status = 0
								rem_from_joblist(self)
								add_to_joblist(self,current_job)
		...
		--							["name"] = "notify",
		--							["status"] = 0,
		--							["message"] = " I could not place the block i wanted to?\n"
		--						}
		--						add_to_joblist(self,notify_job)
							end
...
local function check_inventory_for_space(self)
					local buildera_inv = self:get_inventory()
					--print("DUMP BUILDER INVENTORY: ", dump(buildera_inv))
...
--[[			local mybl = vector.add(get_job_position(self),buildera_town_locations[1])
			local myml = vector.add(get_job_position(self),buildera_town_markers[1])
			local buildera_meta = minetest.get_meta(myml)

			local special_job = {
				["name"] = "movein1",
				["status"] = 0,
				["buildpos"] = mybl,
				["x"] = buildera_meta:get_int("maxx"),
				["y"] = buildera_meta:get_int("maxy"),
				["z"] = buildera_meta:get_int("maxz"),
			}
...
-- ONLY ON GAME LOAD AND CONTINUE
		if buildera_new_start == true then
...
--print("BUILDERA JOBS = ", get_size_joblist(self))
