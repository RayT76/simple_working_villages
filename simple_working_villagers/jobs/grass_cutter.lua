...
local grasscutter_new_start = true
local grasscutter_last_animation = "STAND"
...
local grasscutter_marker = nil



local grasscutter_path_data = nil	-- the builders private path_data


local grasscutter_searching_range = {x = 20, y = 5, z = 20}
local grasscutter_searching_distance = 500
local grasscutter_is_building = false



-- TODO MESSAGING MESSAGES TO BE UPDATED

local grasscutter_callingmessage = 	{ 
			msg = "Calling Builder A",
			}
local grasscutter_callingreply = 	{ 
			msg = "Builder A OK",
			}
local grasscutter_jobreply = 	{ 
			msg = "Builder Finished",
			}
local grasscutter_comemessage = 	{ 
			msg = "Come Here",
			pos = "",
			}
local grasscutter_buildmessage = 	{ 
			msg = "Build This",
			pos = "",
			}
local grasscutter_moveinmessage = 	{ 
			msg = "Movein Here",
			}
...
--	print("MYPOS=", mypos)
--	print("MYDEST=", mydest)
	local grasscutter_myd = mydest
	grasscutter_path_data = pathfinder.plot_movement_to_pos(mypos, grasscutter_myd, false)
	if grasscutter_path_data == nil then
		-- TODO should indicate who cannot find a path
		print(" No Path Found to ", grasscutter_myd)
		self.job_data["pathdata"] = {}
		self.job_data["pathstep"] = -1
		return false
	elseif grasscutter_path_data == false then
...
--		print("          PATHDATA: ",  grasscutter_path_data)
		self.job_data["pathdata"] = grasscutter_path_data
		self.job_data["pathstep"] = 1
		return true
...
local function grasscutter_plantonextto(mypos,mydest)
	local grasscutter_myd = func.get_closest_clear_spot(mypos,mydest)	
	if grasscutter_myd == false or grasscutter_myd == nil then
		grasscutter_myd = mydest
	end
	grasscutter_path_data = pathfinder.plot_movement_to_pos(mypos, grasscutter_myd, false)
	if grasscutter_path_data == nil then
		-- TODO should indicate who cannot find a path
...
local function go_to_here(spos,epos,self)
	...
	while ma_tries > 0 do
		...
		if thedist > 2 then
			grasscutter_plantonextto(curr_loc,epos)
			self:go_to_the(grasscutter_path_data)
		else
			return true
		end
		ma_tries = ma_tries - 1
...
local function check_grasscutter_message(self)
	...
	if func.get_gardener_a_message() ~= nil then 
		...
		if themessage.msg == grasscutter_callingmessage.msg then
			-- I must answer the phone
			print("grasscutter IS ANSWERING THE PHONE")
			func.set_gardener_a_message(grasscutter_callingreply)
		end



		if themessage.msg == grasscutter_comemessage.msg then
			-- I must answer the phone
			print("grasscutter IS BEING CALLED OVER")
			grasscutter_commanddestination = themessage.pos	
			func.set_gardener_a_message(grasscutter_callingreply)
		local goto_job = {
			["name"] = "gotohere",
			["dest"] = themessage.pos,
		...
		if themessage.msg == grasscutter_buildmessage.msg then
			print("grasscutter IS BEING TOLD TO FARM")
				local plantorchard_job = themessage
				plantorchard_job["name"] = "buildthis"
				add_to_joblist(self,plantorchard_job)
			func.get_gardener_a_message(grasscutter_callingreply)
		end


			grasscutter_marker = themessage.pos	
...
--		end
		...
		if themessage.msg == grasscutter_moveinmessage.msg then
			print("grasscutter: IS BEING TOLD TO MOVEIN")
			...
			print("Grasscutter: Chest POS = ", self.pos_data.chest_pos)
			print("Grasscutter: Bed POS = ", self.pos_data.bed_pos)
			print("Grasscutter: Home POS = ", self.pos_data.home_pos)
			--print("grasscutter:DUMP POSDATA", dump(self.pos_data))
...
local function move_in_1(self)
	...
	grasscutter_moveinmessage.bed_pos = bposb
	grasscutter_moveinmessage.food_pos = cposb
	grasscutter_moveinmessage.storage_pos = cposb
	grasscutter_moveinmessage.job_pos = fpos
	grasscutter_moveinmessage.tools_pos = cposb
	grasscutter_moveinmessage.chest_pos = cposb
	grasscutter_moveinmessage.home_pos = hposb
	--print("MAYORA: DUMP MESSAGE = ", dump(grasscutter_moveinmessage))
	func.set_builder_a_message(grasscutter_moveinmessage)
...
local function find_building(p)
	...
	local grasscutter_meta = minetest.get_meta(p)
	if grasscutter_meta:get_string("state") ~= "begun" then
		return false
	end
	local grasscutter_build_pos = simple_working_villages.buildings.get_build_pos(grasscutter_meta)
	if grasscutter_build_pos == nil then
		return false
	end
	if simple_working_villages.buildings.get(grasscutter_build_pos)==nil then
		return false
	end
	return true
end
...
local function check_build_item(self,initem)
		...
		--			print("I Need a Bed Top")
					local grasscutter_inv = self:get_inventory()
					if grasscutter_inv:room_for_item("main", ItemStack(initem)) then
		--				print("adding a bed top")
						grasscutter_inv:add_item("main", ItemStack(initem))
					else
						local msg = "grasscutter at " .. minetest.pos_to_string(self.object:get_pos()) ..
...
--					if grasscutter_inv:room_for_item("main", ItemStack(initem)) then
--						grasscutter_inv:add_item("main", ItemStack(initem))
--						--grasscutter_inv:add_item("main", ItemStack(initem))"beds:bed_top"
--					else
--						local msg = "grasscutter at " .. minetest.pos_to_string(self.object:get_pos()) ..
--							" doesn't have enough inventory space"
--						if self.owner_name then
...
-- TODO CHANGE THIS TO CUT GRASS ---
	...
	elseif current_job.status == 1 then
		...
		gdestination = func.get_closest_clear_spot(self.object:get_pos(),cutter_found_plant_target)	
		local goto_here = {
			["name"] = "gotohere",
			["dest"] = gdestination,
			["status"] = 0
...
local function check_inventory_for_space(self)
					local grasscutter_inv = self:get_inventory()
					--print("DUMP BUILDER INVENTORY: ", dump(grasscutter_inv))
					if grasscutter_inv:room_for_item("main", "dummy:unknown") then
						return true
					else
						return false
					end
...
--[[			local mybl = vector.add(get_job_position(self),grasscutter_town_locations[1])
			local myml = vector.add(get_job_position(self),grasscutter_town_markers[1])
			local grasscutter_meta = minetest.get_meta(myml)

			local special_job = {
				["name"] = "movein1",
				["status"] = 0,
				["buildpos"] = mybl,
				["x"] = grasscutter_meta:get_int("maxx"),
				["y"] = grasscutter_meta:get_int("maxy"),
				["z"] = grasscutter_meta:get_int("maxz"),
			}
...
-- ONLY ON GAME LOAD AND CONTINUE
		if grasscutter_new_start == true then
...
--		end
	...
	--	self:handle_chest(nil, nil)
		...
		check_grasscutter_message(self)
...
-- print("JOBLIST=", dump(grasscutter_joblist))
...
--print("grasscutter JOBS = ", get_size_joblist(self))
