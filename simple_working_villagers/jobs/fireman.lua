...
local fireman_last_animation = "STAND"
...
local fireman_marker = nil



local fireman_path_data = nil	-- the builders private path_data


local fireman_searching_range = {x = 20, y = 5, z = 20}
local fireman_searching_distance = 500
local fireman_is_building = false



-- TODO MESSAGING MESSAGES TO BE UPDATED

local fireman_callingmessage = 	{ 
			msg = "Calling Builder A",
			}
local fireman_callingreply = 	{ 
			msg = "Builder A OK",
			}
local fireman_jobreply = 	{ 
			msg = "Builder Finished",
			}
local fireman_comemessage = 	{ 
			msg = "Come Here",
			pos = "",
			}
local fireman_buildmessage = 	{ 
			msg = "Build This",
			pos = "",
			}
...
--	print("MYPOS=", mypos)
--	print("MYDEST=", mydest)
	local fireman_myd = mydest
	fireman_path_data = pathfinder.plot_movement_to_pos(mypos, fireman_myd, false)
	if fireman_path_data == nil then
		-- TODO should indicate who cannot find a path
		print(" No Path Found to ", fireman_myd)
		self.job_data["pathdata"] = {}
		self.job_data["pathstep"] = -1
		return false
	elseif fireman_path_data == false then
...
--		print("          PATHDATA: ",  fireman_path_data)
		self.job_data["pathdata"] = fireman_path_data
		self.job_data["pathstep"] = 1
		return true
...
local function fireman_plantonextto(mypos,mydest)
	local fireman_myd = func.get_closest_clear_spot(mypos,mydest)	
	if fireman_myd == false or fireman_myd == nil then
		fireman_myd = mydest
	end
	fireman_path_data = pathfinder.plot_movement_to_pos(mypos, fireman_myd, false)
	if fireman_path_data == nil then
...
local function go_to_here(spos,epos,self)
	...
	while ma_tries > 0 do
		...
		if thedist > 2 then
			fireman_plantonextto(curr_loc,epos)
			self:go_to_the(fireman_path_data)
		else
			return true
		end
		ma_tries = ma_tries - 1
...
local function check_fireman_message(self)

	--print("builder checking messages")
	local themessage = nil
	if func.get_fireman_a_message() ~= nil then 
		-- there is a message
		themessage = func.get_fireman_a_message()

		if themessage.msg == fireman_callingmessage.msg then
			-- I must answer the phone
			print("fireman IS ANSWERING THE PHONE")
			func.set_fireman_a_message(fireman_callingreply)
		end



		if themessage.msg == fireman_comemessage.msg then
			-- I must answer the phone
			print("fireman IS BEING CALLED OVER")
			fireman_commanddestination = themessage.pos	
			func.st_fireman_a_message(fireman_callingreply)
		local goto_job = {
			["name"] = "gotohere",
			["dest"] = themessage.pos,
			["status"] = 0
		...
		if themessage.msg == fireman_buildmessage.msg then
			print("fireman IS BEING TOLD TO FARM")
				local plantorchard_job = themessage
				plantorchard_job["name"] = "buildthis"
				add_to_joblist(self,plantorchard_job)
			func.get_fireman_a_message(fireman_callingreply)
		end


			fireman_marker = themessage.pos	
...
--		end
		...
		if themessage.msg == fireman_moveinmessage.msg then
			print("fireman: IS BEING TOLD TO MOVEIN")
			...
			print("fireman: Chest POS = ", self.pos_data.chest_pos)
			print("fireman: Bed POS = ", self.pos_data.bed_pos)
			print("fireman: Home POS = ", self.pos_data.home_pos)
			--print("fireman:DUMP POSDATA", dump(self.pos_data))
...
local function move_in_1(self)
	...
	fireman_moveinmessage.food_pos = cposb
	fireman_moveinmessage.storage_pos = cposb
	fireman_moveinmessage.job_pos = fpos
	fireman_moveinmessage.tools_pos = cposb
	fireman_moveinmessage.chest_pos = cposb
	fireman_moveinmessage.home_pos = hposb
	--print("MAYORA: DUMP MESSAGE = ", dump(fireman_moveinmessage))
	func.set_builder_a_message(fireman_moveinmessage)
...
local function find_building(p)
	...
	local fireman_meta = minetest.get_meta(p)
	if fireman_meta:get_string("state") ~= "begun" then
		return false
	end
	local fireman_build_pos = simple_working_villages.buildings.get_build_pos(fireman_meta)
	if fireman_build_pos == nil then
		return false
	end
	if simple_working_villages.buildings.get(fireman_build_pos)==nil then
		return false
	end
	return true
end
...
local function check_build_item(self,initem)
		...
		--			print("I Need a Bed Top")
					local fireman_inv = self:get_inventory()
					if fireman_inv:room_for_item("main", ItemStack(initem)) then
		--				print("adding a bed top")
						fireman_inv:add_item("main", ItemStack(initem))
					else
						local msg = "fireman at " .. minetest.pos_to_string(self.object:get_pos()) ..
...
--					if fireman_inv:room_for_item("main", ItemStack(initem)) then
--						fireman_inv:add_item("main", ItemStack(initem))
--						--fireman_inv:add_item("main", ItemStack(initem))"beds:bed_top"
--					else
--						local msg = "fireman at " .. minetest.pos_to_string(self.object:get_pos()) ..
--							" doesn't have enough inventory space"
--						if self.owner_name then
...
local function check_inventory_for_space(self)
					local fireman_inv = self:get_inventory()
					--print("DUMP BUILDER INVENTORY: ", dump(fireman_inv))
					if fireman_inv:room_for_item("main", "dummy:unknown") then
						return true
					else
						return false
					end
...
--[[			local mybl = vector.add(get_job_position(self),fireman_town_locations[1])
			local myml = vector.add(get_job_position(self),fireman_town_markers[1])
			local fireman_meta = minetest.get_meta(myml)

			local special_job = {
				["name"] = "movein1",
				["status"] = 0,
				["buildpos"] = mybl,
				["x"] = fireman_meta:get_int("maxx"),
				["y"] = fireman_meta:get_int("maxy"),
				["z"] = fireman_meta:get_int("maxz"),
			}
...
-- ONLY ON GAME LOAD AND CONTINUE
		if fireman_new_start == true then
...
-- print("JOBLIST=", dump(fireman_joblist))
...
--print("fireman JOBS = ", get_size_joblist(self))
